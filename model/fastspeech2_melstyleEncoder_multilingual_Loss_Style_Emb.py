...
class FastSpeech2_StyleEncoder_Multilingual_LossStyle(nn.Module):
    """ FastSpeech2_StyleEncoder_Multilingual """

    def __init__(self, preprocess_config, model_config):
        super(FastSpeech2_StyleEncoder_Multilingual_LossStyle, self).__init__()
    ...
    def get_style_vector(self, mel_target, mel_len=None):
        if mel_len is not None:
            ...
            if mel_target.shape[1] != mel_mask.shape[1]:
                mel_mask = torch.narrow(mel_mask, 1, 0, mel_target.shape[1])
        else:
            mel_mask = None
