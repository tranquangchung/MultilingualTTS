...
from model.modules import ConvNorm, LinearNorm, MultiHeadAttention, get_sinusoid_encoding_table
import pdb
LEAKY_RELU = 0.1
...
class Discriminator(nn.Module):
    ...
    def __init__(self, configs):
        ...
        model_config["discriminator"]["n_speakers"], 
        model_config["discriminator"]["n_mel_channels"], 
        model_config["discriminator"]["style_vector_dim"], 
        model_config["discriminator"]["style_vector_dim"],
        model_config["discriminator"]["style_kernel_size"],
        model_config["discriminator"]["style_head"])

        self.phoneme_D = PhonemeDiscriminator(
        model_config["discriminator"]["n_mel_channels"], 
        model_config["discriminator"]["encoder_hidden"], 
        model_config["discriminator"]["max_seq_len"])

    def forward(self, mels, srcs, ws, sids, mask):
        """
        mels: batch_size X T X 80
        srcs: batch_size X T x 256
        ws: None or 
        sids:
        mask: batch_size X T
        """
