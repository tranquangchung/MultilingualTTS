...
from .modules_diffusion import DenoiserStyle
from .modules_diffusion import DenoiserStyle_LS
...
class GaussianDiffusionShallowStyle_LanguageStyle(nn.Module):
  def __init__(self, preprocess_config, model_config, train_config):
    super().__init__()
    self.denoise_fn = DenoiserStyle_LS(preprocess_config, model_config)
    ...
    with open(
      ...
      self.register_buffer("spec_min",
                           torch.FloatTensor(stats["spec_min"])[None, None, :model_config["denoiser"]["keep_bins"]])
      self.register_buffer("spec_max",
                           torch.FloatTensor(stats["spec_max"])[None, None, :model_config["denoiser"]["keep_bins"]])
    self.aux_mel = None
  ...
  def p_mean_variance(self, x, t, cond, style, lang_vector, clip_denoised: bool):
    noise_pred = self.denoise_fn(x, t, cond, style, lang_vector)
  ...
  @torch.no_grad()
  def p_sample(self, x, t, cond, style, lang_vector, clip_denoised=True, repeat_noise=False):
    b, *_, device = *x.shape, x.device
    model_mean, _, model_log_variance = self.p_mean_variance(x=x, t=t, cond=cond, style=style, lang_vector=lang_vector,clip_denoised=clip_denoised)
  ...
  def p_losses(self, x_start, t, cond, style, lang_vector, noise=None, mask=None):
    noise = default(noise, lambda: torch.randn_like(x_start))

    noised_mel = self.q_sample(x_start=x_start, t=t, noise=noise)
    epsilon = self.denoise_fn(noised_mel, t, cond, style, lang_vector)
  ...
  @torch.no_grad()
  def sampling(self, style, lang_vector, K_step=100):
    ...
    for i in tqdm(reversed(range(0, t)), desc="sample time step", total=t):
      x = self.p_sample(x, torch.full((b,), i, device=device, dtype=torch.long), self.cond, style, lang_vector)
    x = x[:, 0].transpose(1, 2)
    output = self.denorm_spec(x)

    return output

  def forward(self, mel, cond, style, lang_vector, mel_mask, K_step=100):
    ...
    if mel is None:
      output = self.sampling(style, lang_vector, K_step=K_step)
    else:
      ...
      output, epsilon, loss = self.p_losses(x, t, self.cond, style, lang_vector, mask=mel_mask)
  ...
  def forward1(self, mel, cond, style, lang_vector, mel_mask, K_step=100):
    assert self.aux_mel is not None
    b, *_, device = *cond.shape, cond.device
    output = epsilon = None
    loss = t = torch.tensor([0.], device=device, requires_grad=False)
