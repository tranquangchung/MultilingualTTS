...
class GaussianDiffusionShallowStyle(nn.Module):
  def __init__(self, preprocess_config, model_config, train_config):
    super().__init__()
    self.denoise_fn = DenoiserStyle(preprocess_config, model_config)
    self.mel_bins = preprocess_config["preprocessing"]["mel"]["n_mel_channels"]
  ...
  def p_losses(self, x_start, t, cond, style, noise=None, mask=None):
    noise = default(noise, lambda: torch.randn_like(x_start))

    noised_mel = self.q_sample(x_start=x_start, t=t, noise=noise)
    epsilon = self.denoise_fn(noised_mel, t, cond, style)
  ...
  def forward(self, mel, cond, style, mel_mask):
    assert self.aux_mel is not None
    b, *_, device = *cond.shape, cond.device
    output = epsilon = None
    loss = t = torch.tensor([0.], device=device, requires_grad=False)
