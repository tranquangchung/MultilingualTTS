...
class FastSpeech2_StyleEncoder_Multispeaker(nn.Module):
    """ FastSpeech2_StyleEncoder_Multilingual """

    def __init__(self, preprocess_config, model_config):
        super(FastSpeech2_StyleEncoder_Multispeaker, self).__init__()
    ...
    def inference(self, style_vector, src_seq, speaker, src_len=None, max_src_len=None, return_attn=False):
        ...
        #     )
        if self.speaker_emb is not None:
            output = output + self.speaker_emb(speaker).unsqueeze(1).expand(
                -1, max_src_len, -1
            )
