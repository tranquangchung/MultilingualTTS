...
from utils.tools import to_device, log_diffusion, synth_one_sample_multilingual, synth_one_sample_multilingual_diffusion
...
def main(args, configs):
    ...
    # )
    dataset = Dataset(
        "val_test.txt", preprocess_config, train_config, sort=False, drop_last=False
    )
    loader = DataLoader(
        dataset,
        batch_size=2,
    ...
    # model, optimizer = get_model_fastSpeech2_StyleEncoder_MultiLanguage_Difffusion(args, configs, device, train=False)
    # model = get_model_fastSpeech2_StyleEncoder_MultiLanguage_Difffusion(args, configs, device, train=False)
    ...
    while True:
        ...
        for batchs in loader:
            for batch in batchs:
                ...
                #     preprocess_config,
                # )
                # write(f"./wav_output/Wav_diff_V3_8_770k_original.wav", 22050, wav_prediction)

                for K_step in range(10, 101, 10):
                    with torch.no_grad():
                        output = model.forward_2(*(batch[2:]), K_step=K_step)
                    fig, wav_reconstruction, wav_prediction, tag = synth_one_sample_multilingual_diffusion(
                        batch,
                        output,
                        vocoder,
                    ...
                    write(f"./wav_output/Wav_diff_V3_8_770k_{K_step}.wav", 22050, wav_prediction)
                print("DONE")
                exit()
                ...
                #     message2 = "Total Loss: {:.4f}, Mel Loss: {:.4f}, Mel PostNet Loss: {:.4f}, Pitch Loss: {:.4f}, Energy Loss: {:.4f}, Duration Loss: {:.4f}, Noise Loss: {:.4f}".format(
                #         *losses
                #     )
                #
