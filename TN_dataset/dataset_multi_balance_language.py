...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while True:
            lang = self.language_list[self.index_lang % self.num_lang]
            self.index_lang += 1
            speaker = random.choice(list(self.metadata[lang].keys()))
...
if __name__ == "__main__":
    ...
    train_dataset = Dataset(
        "train.txt", preprocess_config, train_config, sort=False, drop_last=True
    )
    val_dataset = Dataset(
