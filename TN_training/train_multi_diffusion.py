...
import numpy as np
from utils.model import get_model, get_vocoder, get_param_num, get_model_fastSpeech2_StyleEncoder, get_model_fastSpeech2_StyleEncoder_MultiLanguage, get_model_fastSpeech2_MultiSpeakers_MultiLangs
from utils.model import get_model_fastSpeech2_StyleEncoder_MultiLanguage_Difffusion
from utils.tools import to_device, log, synth_one_sample, synth_one_sample_multilingual, to_device_diffusion
from model import FastSpeech2Loss_MultiLingual
from model import DiffusionLoss
# from dataset_multi import Dataset
from TN_dataset.dataset_multi_diffusion import Dataset
...
def main(args, configs):
    ...
    # # model, optimizer = get_model_fastSpeech2_MultiSpeakers_MultiLangs(args, configs, device, train=True)
    ...
    Loss = DiffusionLoss(args, preprocess_config, model_config, train_config).to(device)
    ...
    while True:
        ...
        for batchs in loader:
            for batch in batchs:
                batch = to_device_diffusion(batch, device)
                # Forward
                ...
                if step % val_step == 0:
                    model.eval()
                    message = evaluate_multilingual(model, step, configs, val_logger, vocoder)
...
if __name__ == "__main__":
    ...
    user_config = "LibriTTS_StyleSpeech_multilingual_diffusion_test"
    preprocess_config = f"/home/chungtran/Code/C_tutorial/L2/.TTS/FastSpeech2_multilingual/config/{user_config}/preprocess.yaml"
    model_config = f"/home/chungtran/Code/C_tutorial/L2/.TTS/FastSpeech2_multilingual/config/{user_config}/model.yaml"
    train_config = f"/home/chungtran/Code/C_tutorial/L2/.TTS/FastSpeech2_multilingual/config/{user_config}/train.yaml"
    parser.add_argument(
        "-p",
        "--preprocess_config",
        type=str,
