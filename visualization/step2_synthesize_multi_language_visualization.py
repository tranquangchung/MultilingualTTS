...
def get_model():
    ...
    ###########################################################################
    args.restore_step = "100000" #310000
...
if __name__ == "__main__":
    ...
    corpus_path = "/home/ldap-users/s2220411/Code/FastSpeech2_multilingual/visualization/audio/synthesis_audio_interspeech2023_visualization3_5languagesV2"
        ...
        for target_index, target in enumerate(target_duration):
            ...
            for lang_i in language:
                ...
                for text_index, item_text in enumerate(text_language):
                    ...
                    # for visualize
                    os.makedirs(f"{corpus_path}/{model_name}/{target}_second/{language_gt_id}/{target_speaker_name}", exist_ok=True)
                    name_visualize = f"{corpus_path}/{model_name}/{target}_second/{language_gt_id}/{target_speaker_name}/" \
                                     f"{lang_i}__{target_speaker_name}__{filename_speaker}__{language_gt_id}__synthesis__{total_index}.wav"
                    write(name_visualize, 22050, wav_synthesis)
                    # for ground-truth
                    name_visualize = f"{corpus_path}/{model_name}/{target}_second/{language_gt_id}/{target_speaker_name}/" \
                                     f"{lang_i}__{target_speaker_name}__{filename_speaker}__{language_gt_id}__groundtruth__{total_index}.wav"
                    if not os.path.exists(name_visualize):
                        shutil.copy2(path_mosgt, name_visualize)
                    total_index += 1
