...
for lang in languages:
    ...
    for model in models:
      ...
      for dur in duration: # 3, 5, 10
        ...
        for filename in filenames:
          ...
          if model != "groundtruth" and name_language in model_score_pair[model][dur]:
            model_score_pair[model][dur][name_language].append([int(mos), int(sim)])
          if model != "groundtruth" and name_language not in model_score_pair[model][dur]:
            model_score_pair[model][dur][name_language] = []
            model_score_pair[model][dur][name_language].append([int(mos), int(sim)])
  ...
  for model in model_score_pair:
    print("-----")
    for duration in ['3']:
      for name_lang in model_score_pair[model][duration]:
        avg_mos = [mos for [mos, sim] in model_score_pair[model][duration][name_lang]]
        avg_sim = [sim for [mos, sim] in model_score_pair[model][duration][name_lang]]
        if "korean" in name_lang: continue
        target, source = name_lang.split("_")
        print(model, duration, f"{source}->{target}", "MOS: ",round(mean(avg_mos), 3), "SIM: ", round(mean(avg_sim), 3))
