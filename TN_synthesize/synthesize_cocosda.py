...
def synthesize_stylespeech(i, args, model, step, configs, vocoder, batchs, control_values):
    ...
    write(f"wav_output/paper_cocosda/1m/{i}.wav", 22050, wav[0])
...
if __name__ == "__main__":
    ...
    user_config = "VNTTS_MultiSpeaker"
    parser.add_argument(
        "-p",
        "--preprocess_config",
        type=str,
    ...
    args.ref_audio="/data/raw/speech/S_synthesize/Multi_Lang/VN/train_adaptation/Adaptation_paper/lien_1m/0015_lien_lien-104-0010.wav"
    args.restore_step=24000
    args.mode="single"
    print(args)
    ...
    with open('./trash/text_cocosda.txt') as f:
        lines = f.readlines()
        for i, text in enumerate(lines):
            ...
            data_receive = requests.post("https://chuanhoa.ai4med.vn/tts_normalize", json=data)
            phone_chunks = data_receive.json()["phones"]
            phone_chunks = list(chain.from_iterable(phone_chunks))
            texts = np.array([phone_chunks])
            text_lens = np.array([len(texts[0])])
            batchs = [(ids, raw_texts, speakers, texts, text_lens, max(text_lens))]
            control_values = args.pitch_control, args.energy_control, args.duration_control
            synthesize_stylespeech(i, args, model, args.restore_step, configs, vocoder, batchs, control_values)
